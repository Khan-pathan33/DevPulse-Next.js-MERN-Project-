...
export function WebsitePreview({ project, embedded = false }: WebsitePreviewProps) {
  ...
  const [mode, setMode] = useState<"interactive" | "live-iframe">(
    project.liveUrl ? "live-iframe" : "interactive"
  );
        ...
        <div className="min-h-[520px] max-h-[750px] overflow-y-auto bg-[#160c18] text-rose-100 relative">
          {mode === "live-iframe" && project.liveUrl ? (
            <div className="w-full relative flex flex-col">
              {/* Direct Website Launch Strip */}
              <div className="p-3 bg-[#1d101f] border-b border-rose-900/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
                <div className="flex items-center gap-2 truncate">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0 animate-pulse" />
                  <span className="text-rose-300/80 font-semibold">Real Website:</span>
                  <a
                    href={project.liveUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-mono text-white underline hover:text-[#f3c1cf] truncate"
                  >
                    {project.liveUrl}
                  </a>
                </div>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-3.5 py-1.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-[#c06c84] to-[#d8829d] hover:brightness-110 flex items-center gap-1.5 shrink-0 shadow-sm transition-all"
                >
                  <span>Open Full Real Website ↗</span>
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
              </div>
            ...
            <div className="p-6 space-y-6">
              ...
              {/* 7. GYM & FITNESS WEB APP (User's Published Project) */}
              {(project.slug === "next-js-project-3377" || project.description.toLowerCase().includes("gym")) && (
                <div className="space-y-6">
                  {/* Gym Hero Banner */}
                  <div className="p-6 rounded-3xl bg-gradient-to-r from-[#2a1324] via-[#1f1022] to-[#160c18] border border-rose-900/40 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl">
                    <div className="space-y-2 text-center sm:text-left">
                      <span className="px-3 py-1 rounded-full text-[11px] font-bold bg-[#d8829d]/20 text-[#f3c1cf] border border-[#d8829d]/30">
                        ⚡ IronPulse Fitness & Gym Web App
                      </span>
                      <h4 className="text-xl font-black text-white">Next.js Strength & Conditioning Suite</h4>
                      <p className="text-xs text-rose-200/70 max-w-md">
                        Track progressive overload, calculate 1RM PRs, and explore targeted muscle group routines.
                      </p>
                    </div>
                    {project.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-[#c06c84] to-[#d8829d] hover:brightness-110 shadow-lg flex items-center gap-2 shrink-0"
                      >
                        <span>Open Live Gym App</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      { name: "Push Day: Hypertrophy", exercises: "Flat Bench, Incline DB, Tricep Dips", duration: "55 mins", intensity: "High" },
                      { name: "Pull Day: Upper Back & Biceps", exercises: "Deadlifts, Lat Pulldowns, Preacher Curls", duration: "60 mins", intensity: "Heavy" },
                      { name: "Leg Day: Squats & Core", exercises: "Barbell Back Squat, Romanian Deadlift, Calf Raises", duration: "50 mins", intensity: "Max" },
                    ].map((routine) => (
                      <div
                        key={routine.name}
                        className="p-4 rounded-2xl bg-[#1e1120] border border-rose-400/20 hover:border-[#d8829d] transition-all space-y-3"
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] uppercase font-bold text-[#e2a76f] bg-[#e2a76f]/15 px-2 py-0.5 rounded">
                            {routine.intensity}
                          </span>
                          <span className="text-[11px] font-mono text-rose-300/60">{routine.duration}</span>
                        </div>
                        <h5 className="text-xs font-bold text-white leading-snug">{routine.name}</h5>
                        <p className="text-[11px] text-rose-200/70">{routine.exercises}</p>
                        <a
                          href={project.liveUrl || "https://gym-exercise.netlify.app"}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full py-1.5 rounded-xl text-xs font-semibold text-rose-200 hover:text-white bg-[#261528] hover:bg-[#341b37] flex items-center justify-center gap-1.5 transition-colors"
                        >
              ...
              {![
                ...
                "next-js-project-3377",
              ].includes(project.slug) && !project.description.toLowerCase().includes("gym") && (
